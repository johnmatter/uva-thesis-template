...
As such, these elements are set to zero during optimization and taken from a
COSY model during reconstruction.
...
and
$o^{rec}_e$ the reconstructed estimate
from the corresponding optics matrix $O_{ijkml}$.
The $\chi^2$ for a set of matrices $O_{ijklm}$ is
